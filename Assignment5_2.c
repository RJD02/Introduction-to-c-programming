...
int checking(int n1, int n2, int p)     //Here n1 = f(n - 1), n2 = f(n - 2) and p = number that we have to find whether it exists in the sequence or not
{
   if (n1 >= p)
   {
      return abs(p - n1);               //abs() returns the absolute value, that is the |n1 - n2|, mod of the difference
   }
   if (n2 >= p)
   {
      return abs(p - n2);               //If either of the n1, or n2 are close to p then we will simply return their difference
   }
